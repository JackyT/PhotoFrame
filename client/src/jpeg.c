...
int read_JPEG_file (char * filename, fb_info * jpeg_inf)
{
	...
	jpeg_inf->w = w;
	jpeg_inf->h = h;
	jpeg_inf->bpp = bpp ;//每个像素有多少个字节
	retbuf = malloc(w * h * bpp);
	jpeg_inf->fbmem = retbuf;
	...
	while (cinfo.output_scanline < cinfo.output_height)
	{
		...
		memcpy(retbuf + y * row_stride, buffer[0], row_stride) ;
		y++;
	}
